...
public:
    PlayStateCB (ros::NodeHandle& nh, rfsm::StateMachine& fsm, QTrobotServiceHelper& srvHelper):
        rfsm(fsm), serviceHelper(srvHelper), with_audio_files(false) {
        srand(time(0));
        ...
        if(with_audio_files) {
            ...
            if(!nh.getParam("/qt_memgame_app/speech_message_file", speech_message_file)) {
                ROS_WARN_STREAM("Cannot find param  /qt_memgame_app/speech_message_file.");
            }
