...
from engines.fastchat import FastChat
...
class QTChatBot():
    ...
    def __init__(self):                
        ...
        self.model_engine = rospy.get_param("/offline_conversation/engine", "fastchat")
        self.asr_engine = rospy.get_param("/offline_conversation/asr", "riva")
        self.sentiment = rospy.get_param("/offline_conversation/sentiment", False)        
        self.tts_voice = rospy.get_param("/offline_conversation/tts_voice", 'en_US')        

        if self.model_engine == 'fastchat':
            self.aimodel = FastChat()
        else:
        ...
        # setup asr engine
        if not self.asr_engine in ['vosk', 'riva']:
        ...
        rospy.loginfo("waiting for '/qt_robot/speech/recognize'...")
        ...
        # set the voice
        status = self.speechConfig(self.tts_voice, 0, 0)
    ...
    def start(self):
        ...
        while not rospy.is_shutdown() and not self.finish:            
            ...
            if self.model_engine == 'fastchat':
                results = bs.sync([
                    (0, lambda: self.aimodel.generate(prompt, lambda txt: self.speak(txt))),
        ...
        print("offline_conversation_node Stopping!")
        self.finish = False


if __name__ == "__main__":
   
    rospy.init_node('offline_conversation_node')
    rospy.loginfo("offline_conversation_node started!")                               
    speechBot = QTChatBot()
    speechBot.start()
    rospy.spin()
    rospy.loginfo("offline_conversation_node is ready!")    
