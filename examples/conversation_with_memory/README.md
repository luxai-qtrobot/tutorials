...
In the config file, you can choose among vosk, riva and gspeech options. Please choose only one, for example `asr: "riva" `.  These are the prerequisites for making ASR work:
...
We use LangChain to work with vector stores and embeddings. The follwing requirements need to be installed:

- langchain
- langchain-community
- langchain_openai (only if you want to work with OpenAI embeddings)
- unstructured
- tiktoken
- faiss-cpu (even if you work on a GPU)

To work on QTrobot AI@Edge, you will need the NVIDIA version of Torch. Downoad the right wheel from [NVIDIA official versions](https://forums.developer.nvidia.com/t/pytorch-for-jetson/72048). Please install the right version BEFORE you install sentence-embeddings. We ue the following version in our example:

- export TORCH_INSTALL=path-to-download/torch-2.1.0a0+41361538.nv23.06-cp38-cp38-linux_aarch64.whl
- python3 -m pip install --no-cache $TORCH_INSTALL

If you work on the GPU version of the robot, you install `sentence-transformers` *after* you installed Torch. If you work on a CPU version of QTrobot, you can install `sentence-transformers` directly, it will also install Torch (only CPU version will be then available).

## Setup languge generation
...
To work on QTrobot AI@Edge, you will need the NVIDIA version of Torch:
- export TORCH_INSTALL=~/robot/torch/torch-2.1.0a0+41361538.nv23.06-cp38-cp38-linux_aarch64.whl
- python3 -m pip install --no-cache $TORCH_INSTALL

The following dependencies need to be installed for Fastchat:

- fschat[model_worker] (please install "fschat[model_worker]==0.2.30" if you work with Python 3.8, newer versions did not work for us)
- psutil
- accelerate
- protobuf
...
In this example, the origin of the robot is the planet Electoria. This information is stored in a file, and the agent's character can be easily changed by modifying the text in the file `data/qtrobot-story.txt`. This message will be passed to the generative model as a `system_message`.
...
All conversations will be automatically stored in the `logs` folder. You can use them for quality assessment and/or simulating long-term memory. 

## Config, build and launch

### Config and start script

The start script `scripts/start_conversation_with_memory.sh` activates the virtual environment named `.venv` (with dot). If your environment has a different name, please replace it in this line:
```
source ~/catkin_ws/src/conversation_with_memory/.venv/bin/activate
```

### Build 
Make a link (or copy) to `conversation_with-memory` in catkin workspace source directory and build it:
...
Then launch:
```
$ roslaunch conversation_with_memory.launch
```